...
def read_receiver() -> Dict[str, Any]:
    ...
    if receiver_url:
        ...
        if settings["source_type"] == "url" and settings["aircraft_url"]:
            payload = read_payload_from_url(settings["aircraft_url"].replace("aircraft.json", "receiver.json"))
        elif settings["source_type"] == "file" and settings["aircraft_path"]:
            payload = read_payload_from_file(settings["aircraft_path"].replace("aircraft.json", "receiver.json"))
        else:
